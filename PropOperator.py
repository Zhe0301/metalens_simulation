...
class PropOperator:
    def __init__(self, Grid, wavelength_vacuum, dist, refractive_index=1, method='AS',
        ...
        if self.method == "AS" or self.method == "BL-AS":  # 使用角谱理论
            if gpu_acceleration:
                grid_d2_fft_x2 = cp.asarray(Grid.d2_fft_x)
                grid_d2_fft_y2 = cp.asarray(Grid.d2_fft_y)
                grid_d2_fft_x2 = cp.power(grid_d2_fft_x2, 2)
                grid_d2_fft_y2 = cp.power(grid_d2_fft_y2, 2)
            else:
                grid_d2_fft_x2 = xp.power(Grid.d2_fft_x, 2)
                grid_d2_fft_y2 = xp.power(Grid.d2_fft_y, 2)
        elif self.method == "FFT-DI":
            if gpu_acceleration:
                axis = cp.asarray(Grid.axis)
            else:
                axis = Grid.axis
        if self.method == "AS":
            """
            ...
            rmax = xp.sqrt((xp.max(axis) ** 2) + (xp.max(axis) ** 2))
            dr_ideal = xp.sqrt(self.wavelength ** 2 + rmax ** 2 + 2 *
                               self.wavelength * xp.sqrt(rmax ** 2 + self.dist ** 2)) - rmax  # 理想采样间隔
            ...
            # 生成一个(2N-1)×(2N-1)矩阵
            X_tmp_1D = xp.linspace(2 * axis[0], -2 * axis[0], 2 * Grid.num_points - 1)
            Y_tmp_1D = xp.linspace(2 * axis[0], -2 * axis[0], 2 * Grid.num_points - 1)
            X_tmp_2D, Y_tmp_2D = xp.meshgrid(X_tmp_1D, Y_tmp_1D)
            tmp_r = np.sqrt(X_tmp_2D ** 2 + Y_tmp_2D ** 2 + self.dist ** 2)
            self.mat_DI = 1 / (2 * xp.pi) * xp.exp(1j * self.k_prop * tmp_r) * self.dist / tmp_r ** 2 * (
    ...
    def prop(self, complex_amplitude):
        ...
        if self.gpu_acceleration:
            complex_amplitude = cp.asarray(complex_amplitude)
        ...
        if self.method == "AS" or self.method == "BL-AS":  # 使用角谱理论
            ...
            # self.e_out = S[:N, :N]
        if self.gpu_acceleration:
            self.e_out = cp.asnumpy(self.e_out)
        return self.e_out
