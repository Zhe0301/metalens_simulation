...
def three_element_zoom_system(S, L1, L2, L3, G, d_lens, d_12, d_23, d_bfl, efl, refractive_index,
                              save_path, sampling_point=200, interval=1, method='AS', show=False,
    ...
    intensity = np.power(np.abs(e_6), 2)  # 像面光强
    mid_index_0 = len(G.axis) // 2  # 坐标中心位置
    """像面绘图"""
    plt.figure(figsize=(15, 13))
    ax = plt.subplot(2, 2, 1)
    ax.set_aspect(aspect=1)
    plt.pcolormesh(G.d2_x, G.d2_y, intensity, cmap="rainbow")
    cb = plt.colorbar()
    ...
    plt.plot(G.axis, np.abs(intensity[mid_index_0, :]), linewidth=2, label=r'$x$-axis Cross Section ')  # 截面图 坐标单位 μm
    ...
    ax.set_aspect(aspect=1)
    plt.pcolormesh(G.d2_x, G.d2_y, np.log10(intensity), cmap="rainbow")
    cb = plt.colorbar()
    cb.set_label(r'$\log_{10}{\mathrm{Intensity}}$')  # 给colorbar添加标题
    plt.title('Image')
    plt.xlabel(r'$x$(mm)')
    plt.ylabel(r'$y$(mm)')
    plt.subplot(2, 2, 4)
    plt.plot(G.axis, np.log10(intensity[mid_index_0, :]))  # 截面图 坐标单位 μm
    plt.title(r'$x$-axis Cross Section')
    plt.xlabel(r'$x$(mm)')
    plt.ylabel(r'$\log_{10}{\mathrm{Intensity}}$')
    plt.grid()
    plt.tight_layout()
    ...
    plt.close()
    """MTF计算"""
    mtf_x, mtf_y, _ = calculate_mtf(np.power(np.abs(e_6), 2), G.step, gpu_acceleration=gpu_acceleration)
    # MTF绘图
    plt.figure(figsize=(9, 7))
    plt.title('Modulation transfer function')
    plt.xlabel('Frequency(lp/mm)')
    plt.ylabel(r'MTF')
    plt.plot(G.axis_fft[len(mtf_x):], mtf_x, linewidth=2, label=r'$x$', color='royalblue')
    plt.plot(G.axis_fft[len(mtf_x):], mtf_y, linewidth=2, label=r'$y$', linestyle="--", color='orange')
    plt.grid()
    plt.legend()
    if show:
        plt.show()
    ...
    center_x, center_y, r_x_p, r_x_n, r_y_p, r_y_n = calculate_fwhm(intensity, G, centrosymmetry=True,
                                                                    gpu_acceleration=gpu_acceleration)
    fwhm = r_x_p + r_x_n
    logger.info("full width at half maximum is {:.2f} μm".format(fwhm * 1000))
    source_energy = np.trapz(np.trapz(np.power(np.abs(S.complex_amplitude * L1.mask), 2), G.axis), G.axis)  # 光源入射的能量
    image_energy = np.trapz(np.trapz(intensity, G.axis, axis=1), G.axis)  # 像面上的能量
    ...
    radius, energy_ratio = calculate_enclosed_energy_ratio(intensity, G, center_x, center_y, max_radius=12e-3,
                                                           total_energy=0, min_points=256,
                                                           gpu_acceleration=gpu_acceleration)

    # 圈入能量和半高宽插值绘图

    f = PchipInterpolator(radius, energy_ratio)  # 单调三次样条
    radius_interp = np.linspace(np.min(radius), np.max(radius), 3 * np.size(radius))  # 对数据进行上倍插值
    energy_ratio_interp = f(radius_interp)

    plt.figure(figsize=(9, 7))

    plt.plot(radius_interp * 1000, energy_ratio_interp, linewidth=2, color='royalblue')
    plt.axvline(x=fwhm / 2 * 1000, color='r', linestyle='--', label=r'FWHM = {:.2f}$\mu$m'.format(fwhm * 1000))
    ...
    # 放大像面并插值绘图
    x_interp, y_interp, I_interp = extract_spot(intensity, G, center_x, center_y, max_radius=12e-3, interp_factor=2,
                                                gpu_acceleration=False)
    # 在新网格上插值
    plt.figure(figsize=(15, 7))
    ax = plt.subplot(1, 2, 1)
    ax.set_aspect(aspect=1)
    plt.pcolormesh(x_interp * 1000, y_interp * 1000, I_interp, cmap="rainbow")  # 插值图 坐标单位 μm
    cb = plt.colorbar()
    cb.set_label('Intensity')  # 给colorbar添加标题
    cb.formatter = ticker.ScalarFormatter(useMathText=True)
    cb.formatter.set_scientific(True)
    ...
    plt.plot(x_interp * 1000, I_interp[np.size(y_interp) // 2, :], linewidth=2, label=r'$x$-axis',
             color='royalblue')  # 截面图 坐标单位 μm
    plt.plot(x_interp * 1000, I_interp[:, np.size(x_interp) // 2], linewidth=2, label=r'$y$-axis',
             linestyle="--", color='orange')  # 截面图 坐标单位 μm
    plt.legend()
    plt.title(r'Image Cross Section')
    plt.xlabel(r'$x$ or $y$($\mu$m)')
    plt.ylabel('Intensity')
    plt.grid()
    plt.tight_layout()
    ...
    if interval > 0 and sampling_point > 0:
        ...
        if show:
            plt.show()
        plt.close()
        with h5py.File(save_path + "e_5_f_{:.1f}.h5{:.1f}.h5".format(efl), 'w') as f:
